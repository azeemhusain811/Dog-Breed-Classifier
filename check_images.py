...
def get_input_args():
    ...
    parser.add_argument('--dogfile', type=str, default='dognames.txt', help='text files that has dog names')
...
def classify_images(images_dir, petlabel_dic, model):
    ...
    for key in petlabel_dic:
        ...
        if found>=0:
            if ( (found==0 and len(truth)==len(classifier_label)) or
                 ( ( (found==0) or (classifier_label[found-1]==" ") ) and
                   ( (found + len(truth) == len(classifier_label)) or
                     (classifier_label[found + len(truth):found + len(truth) +1] in
                      (",", " "))
                   )
                 )
               ):
                if key not in results_dic:
                    results_dic[key] = [truth, classifier_label, 1]
            else:
                if key not in results_dic:
                    results_dic[key] = [truth, classifier_label, 0]
        else:
            if key not in results_dic:
                results_dic[key] = [truth, classifier_label, 0]
...
def calculates_results_stats(results_dic):
    ...
    results_stats['n_notdogs_img'] = results_stats['n_images'] - results_stats['n_dogs_image']
    results_stats['pct_match'] = (results_stats['n_match']/results_stats['n_images'])*100.0
    results_stats['pct_correct_dogs'] = (results_stats['n_correct_dogs']/results_stats['n_dogs_image'])*100.0
    results_stats['pct_correct_breed'] = (results_stats['n_correct_breed']/results_stats['n_dogs_image'])*100.0
...
def print_results(results_dic, results_stats, model, print_incorrect_dogs=False, print_incorrect_breed=False):
    ...
    print('%20s: %3d' % ('N Dog Images', results_stats['n_dogs_image']))
    print('%20s: %3d' % ('N Not-Dog Images', results_stats['n_notdogs_img']))

    for key in results_stats:
        if key[0]=='p':
            print('%20s: %3d' % (key, results_stats[key]))
